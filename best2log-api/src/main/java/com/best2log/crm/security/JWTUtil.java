...
public class JWTUtil {
    ...
    public String generateTokenWithUserData(Funcionario funcionario) throws IllegalArgumentException, JWTCreationException {
        ObjectMapper mapper = new ObjectMapper();
        String userJson = null;
        try {
            userJson = mapper.writeValueAsString(funcionario);
        ...
        return JWT.create()
                .withSubject(subject)
                .withClaim("funcionario", userJson)
                .withIssuedAt(new Date())
                .withIssuer(companyProjectName)
                .sign(Algorithm.HMAC256(secret));
    }
    ...
    public String validateTokenAndRetrieveSubject(String token) throws Exception {
        ...
        try {
            user = mapper.readValue(jwt.getClaim("funcionario").asString(), Funcionario.class);
        } catch (JsonProcessingException e) {
        ...
        return user.getEmailFuncionario();
    }
