...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Autowired
	private FuncionarioDetailsServiceImpl uds;
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable() // Desabilitando o csrf
				...
				.and().authorizeHttpRequests() // Autorizando requisicoes de entrada
				.antMatchers("/auth/**", "/swagger-ui/**", "/v3/api-docs/**", "/actuator/**").permitAll() // Autorizando
																											// requisicoes
																											// autenticadas
				.antMatchers("/user/**").hasRole("USER") // Autorizando apenas usuarios com o perfil "USER" a utilizar
															// esse endpoint (nesse caso, path /user/ e uri´s
															// subsequentes
				.anyRequest().authenticated() // Todas as requisicoes, exceto para as definidas acima, precisarao ser
												// autenticadas
				.and().userDetailsService(uds) // Setando o servico "user details" (do modulo Security do Spring) para
												// essa implementacao customizada
				.exceptionHandling().authenticationEntryPoint(
						...
						(request, response, authException) -> response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
								"Unauthorized"))
				.and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS); // Configurando a
																									// Sessao para que
																									// cada requisicao
																									// seja independente
																									// (stateless)
